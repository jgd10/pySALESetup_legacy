...
class Grain:
    ...
    def __init__(self, eqr=10., rot=0., shape='circle', 
        ...
        Args:
            ...
            elps_eccen:  float; eccentricity of the ellipse (0 <= e < 1)
        ...
        if self.shape == 'circle' or self.shape =='sphere':
            ...
            assert elps_eccen is not None, "ERROR: ellipse creation requires an eccentrcity to be given"
            self.mesh = psf.grainfromEllipse(self.equiv_rad,self.angle,elps_eccen)
            self.area = np.sum(self.mesh)
            self.eccentricity = elps_eccen
            self.radius = self.equiv_rad
    ...
    def remove(self):
        """
        reverse of place(); removes grain instance from the mesh but(!) does not delete the instance.
        """
        Is,Js,i_,j_ = self._cropGrain(self.x,self.y,self.hostmesh.x,self.hostmesh.y)
        for p in range(Js[1]-Js[0]):
            for o in range(Is[1]-Is[0]):
                cell = self.hostmesh.materials[self.mat-1,o+i_[0],p+j_[0]]
                if cell>0.:
                    self.hostmesh.materials[self.mat-1,o+i_[0],p+j_[0]] = 0.
                    self.hostmesh.mesh[o+i_[0],p+j_[0]] = 0.
        return
...
class Ensemble:
    ...
    def Del(self,particle,x=None,y=None):
        """
        Remove a grain from the Ensemble. If two Grain instances have been added to the
        same ensemble, del will only remove the first one it encounters!

        Args:
            particle: Grain
        """
        i = self.grains.index(particle)
        self.grains.pop(i)
        self.rots.pop(i)
        self.radii.pop(i)
        self.xc.pop(i)
