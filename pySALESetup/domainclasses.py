...
class Mesh:
    ...
    def lineEdges(self,m,cells=1,Edges=[True,True,True,True]):
        """
        Line all edges of the mesh with materials specified by m.
        as usual, m = -1 implies void 
        Args:
            m = int or list of ints in structure [Nmat,Smat,Wmat,Emat]
            thickness = int (cells) or list of ints [Nthick,Sthick,Wthick,Ethick]
            overwrite = bool 
            Edges = list of bools; corresponding to N S E W edges
        Output:
            no output
        """
        # thickness (in cells)
        thk = cells 
        if type(m) == int: m = [m,m,m,m]
        if type(thk) == int: thk = [thk,thk,thk,thk]
        assert len(m) == len(thk) and len(thk) == len(Edges), "ERROR: the same number of thicknesses as materials must be supplied"
        assert (0 in m) == (False), "ERROR: lineEdges does not support selective overwriting, yet"
        if Edges[0]: 
            t = thk[0]
            if m[0] != -1: 
                temp_materials = np.copy(self.materials[m[0]-1,:,-t:])
                summed_mats = np.sum(self.materials[:,:,-t:],axis=0)
                temp_materials[(summed_mats<1.)] = 1.
                temp_2 = summed_mats*temp_materials
                temp_materials -= temp_2
                self.materials[:,:,-t:] += temp_materials
            else:
                self.materials[:,:,-t:] *= 0.
        if Edges[1]: 
            t = thk[1]
            if m[1] != -1: 
                temp_materials = np.copy(self.materials[m[1]-1,:,:t])
                summed_mats = np.sum(self.materials[:,:,:t],axis=0)
                temp_materials[(summed_mats<1.)] = 1.
                temp_2 = summed_mats*temp_materials
                temp_materials -= temp_2
        ...
        if Edges[2]: 
            t = thk[2]
            if m[2] != -1: 
                temp_materials = np.copy(self.materials[m[2]-1,-t:,:])
                summed_mats = np.sum(self.materials[:,-t:,:],axis=0)
                temp_materials[(summed_mats<1.)] = 1.
                temp_2 = summed_mats*temp_materials
                temp_materials -= temp_2
        ...
        if Edges[3]: 
            t = thk[3]
            if m[3] != -1: 
                temp_materials = np.copy(self.materials[m[3]-1,:t,:])
                summed_mats = np.sum(self.materials[:,:t,:],axis=0)
                temp_materials[(summed_mats<1.)] = 1.
                temp_2 = summed_mats*temp_materials
                temp_materials -= temp_2
...
class CombinedMesh:
    ...
    def __init__(self,mesh,N=None,S=None,E=None,W=None,ExtendFromMesh=False):
        ...
        self.yi, self.xi = np.meshgrid(self.yc,self.xc)
        # reassign main mesh
        self.materials[:, self.is_:self.ie_+1, self.js_:self.je_+1] = mesh.materials
        self.VX[self.is_:self.ie_+1, self.js_:self.je_+1] = mesh.VX
        self.VY[self.is_:self.ie_+1, self.js_:self.je_+1] = mesh.VY
        ...
        if ExtendFromMesh:
            for i,ii in zip(range(self.is_,self.ie_),range(mesh.x)):
                if S is not None:
                    if np.amax(mesh.materials[:,ii,0])>0.:
                        m = np.where(mesh.materials[:,ii,0]==1.)[0][0]
                        self.materials[m,i,:self.js_] *= 0.
                        self.materials[m,i,:self.js_] = 1.
                    else:
                        self.materials[:,i,:self.js_] *= 0.
                if N is not None:
                    if np.amax(mesh.materials[:,i,-1])>0.:
                        m = np.where(mesh.materials[:,i,-1]==1.)[0][0]
                        self.materials[m,i,self.je_:] *= 0.
                        self.materials[m,i,self.je_:] = 1.
                    else:
                        self.materials[:,i,self.je_:] *= 0.

            for j,jj in zip(range(self.js_,self.je_),range(mesh.y)):
                if W is not None:
                    if np.amax(mesh.materials[:,0,jj])>0.:
                        m = np.where(mesh.materials[:,0,jj]==1.)[0][0]
                        self.materials[m,:self.is_,j] *= 0.
                        self.materials[m,:self.is_,j] = 1.
                    else:
                        self.materials[:,:self.is_,j] *= 0.
                if E is not None:
                    if np.amax(mesh.materials[:,-1,jj])>0.:
                        m = np.where(mesh.materials[:,-1,jj]==1.)[0][0]
                        self.materials[m,self.ie_:,j] *= 0.
                        self.materials[m,self.ie_:,j] = 1.
                    else:
                        self.materials[:,self.ie_:,j] *= 0.
        return
    ...
    def blockVel(self,kmin,kmax,vel,velaxis=0,horizontal=True):
        ...
        if horizontal:
            ...
            if velaxis == 0:
                self.VX[kmin:kmax,self.js_:self.je_+1] = vel
            elif velaxis == 1:
                self.VY[kmin:kmax,self.js_:self.je_+1] = vel
            elif velaxis == 2:
                assert len(vel)==2, "ERROR: when giving both a value, vel must be of form vel = [xvel,yvel]"
                self.VX[kmin:kmax,self.js_:self.je_+1] = vel[0]
                self.VY[kmin:kmax,self.js_:self.je_+1] = vel[1]
        return
...
class SetupInp:
    ...
    def populate_fromMesh(self,MM,N=None,S=None,E=None,W=None):
        """ 
        popluate the dictionary from an existing Mesh instance 
